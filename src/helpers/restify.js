...
restifyHelper.httpError = function(err) {
  ...
  if (err === appError.createStudentError ||
    ...
    err === appError.sendingMailAAError ||
    err === appError.confirmUserError ||
    err === appError.getUserError ||
    err === appError.getCertificateBatchError ||
    err === appError.createFundingAddressesError ||
    err === appError.callFundsServiceError ||
    err === appError.createCerticateBatchesError ||
    err === appError.updateCertificateFundsError ||
    err === appError.createCertificatesError ||
    err === appError.proofValidationFail ||
    err === appError.validateCertificateError ||
    err === appError.revokingAddressError ||
    err === appError.updateStudentError ||
    err === appError.getStudentError ||
    err === appError.getCertificatesByStudentIdError) {
    httpError = new restify.errors.InternalServerError(err);
  } else if (err === appError.loginUserError ||
           err === appError.userInactiveError) {
  ...
  } else if (err === appError.invitationInexistentError ||
             err === appError.userInexistentError) {
    httpError = new restify.errors.InvalidContentError(err);
  } else if( err === appError.certificationNotFound ||
             err === appError.notFoundStudentError) {
